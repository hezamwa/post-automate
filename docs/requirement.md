...
| ID | Requirement | Priority |
|----|-------------|----------|
| FR-6.12 | For every article draft, the pipeline MUST generate an **X.com short version** (text only, in the creator's voice, within X's length limits), reviewed together with the article. Publishing to X.com itself is out of scope for v1 — the text is stored with the post for manual use. | MUST |
...
| FR-7.9 | The review screen MUST offer **request-revision**: free-text instructions regenerate the draft via the same article route with all guardrails intact; the X version and translation are re-derived (hero image kept unless the instructions address it); the draft returns to pending review. Maximum **3 revisions per draft**, each budget-metered. Revision instructions are stored and feed profile refinement (FR-6.10). A **change-angle** option regenerates from one of the other stored angle proposals instead. **[OD-24 — resolved]** | MUST |
...
| AR-10.6 | The database MUST be a **managed PostgreSQL** (e.g., Neon or Supabase) accessed via Cloudflare **Hyperdrive**. D1/SQLite is NOT acceptable for the relational model in §9. Data access uses a TypeScript SQL toolkit (e.g., Drizzle or Kysely), preserving CQRS at module level: command handlers write, query handlers read. | MUST |
