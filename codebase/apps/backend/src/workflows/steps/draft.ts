...
import { angleSchema, articleResultSchema } from "../../modules/generation/types";
...
export const draft = defineStep({
  name: "draft",
  input: z.object({ topic: candidateRefSchema, angle: angleSchema, revision: draftRevisionSchema.optional() }),
  output: articleResultSchema,
  bills: "article",
  retries: RETRY.ai,
  nonRetryable: [ComplianceRefusalError],
  run: async (ctx, { topic, angle, revision }) => {
    const db = createDb(ctx.env);
    if (revision) await setDraftStatus(db, revision.draftId, "revising");
    const rewrite =
      revision?.instructions != null && revision.currentMarkdown != null
        ...
        : undefined;
    const result = await writeArticle(ctx.env, db, moduleCtx(ctx), topic, angle, rewrite);
