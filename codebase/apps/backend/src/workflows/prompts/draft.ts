...
function articleRequest(topic: TopicBrief, angle: Angle): string {
...
export function buildDraftPrompt(input: DraftPromptInput): PromptSpec {
  ...
  return {
    ...
    messages: [
      { role: "user", content: input.revision ? revisionRequest(input.revision) : articleRequest(input.topic, input.angle) },
    ],
